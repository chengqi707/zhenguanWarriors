...
let n = 0;
const shot = async name => { n++; await page.screenshot({ path: `${OUT}e${String(n).padStart(2, '0')}_${name}.png` }); console.log('📸', name); };
...
await shot('levelselect_fit');
await page.getByText('第1关').click();
await page.waitForSelector('.story-skip');
await shot('story_portrait');
await page.getByText('跳过').click();
await page.waitForSelector('.hero-list, .hero-grid');
await shot('heroselect_grid');
await page.getByText('确认阵容').click();
await page.waitForSelector('.equip-main');
await shot('equip_traits');
await page.getByText('⚔ 开始战斗').click();
await page.waitForFunction(() => !!window.__scene, null, { timeout: 8000 });
await page.waitForTimeout(1800);
await shot('battle_sprites');

// 放大看单位模型（滚轮放大到部队集群）
await page.evaluate(() => {
  const s = window.__scene;
  const c = s.canvas.getBoundingClientRect();
  s.camera.zoomAt(c.width * 0.3, c.height * 0.5, 2.2);
});
await page.waitForTimeout(400);
await shot('battle_sprites_zoomed');

// 点空地格 → 地形信息条
await page.evaluate(() => {
  const s = window.__scene;
  const c = s.canvas.getBoundingClientRect();
  s.camera.zoomAt(c.width / 2, c.height / 2, 1 / 2.2);
});
await page.waitForTimeout(300);
await clickCell(7, 5); // 林地格（第1关中部有树林）
await page.waitForTimeout(400);
await shot('terrain_info');
...
await page.waitForTimeout(400);
await shot('unit_trait');

console.log('✅ 战场可视化验证完成');
await browser.close();
